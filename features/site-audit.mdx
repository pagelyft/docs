...
<Steps>
  ...
  <Step title="Start the crawl">
    Return to the Site Audit page and click **Run Audit**. The crawl runs in the background — you can navigate away and return later. RankLyft checks for results every 10 seconds while you're on the page.
  </Step>
</Steps>

Scheduled audits run automatically based on the schedule you configure in Site Settings.

<Warning>
  JavaScript rendering takes significantly longer and uses more of your DataForSEO balance. Only enable it if the site requires JavaScript to display its content. Most static or server-rendered sites do not need it.
</Warning>
...
After starting a crawl, RankLyft shows a progress indicator. Depending on the number of pages and whether JS rendering is enabled, a crawl typically completes in 2–15 minutes. You do not need to keep the tab open — results will be saved when the crawl finishes.
