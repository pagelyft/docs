...
An optional label that replaces the domain throughout the RankLyft UI. Useful when managing multiple sites for the same client or when the domain isn't descriptive (for example, labeling `acme.com` as "Acme Corp").
...
<Warning>
  Removing a site hides it and all its data. This action cannot be undone from the UI. Contact your PageLyft admin if you need to restore a removed site.
</Warning>
...
|---|---|---|
| Include subdomains | Off | Crawl pages on subdomains (for example, `blog.example.com`) in addition to the root domain |
