...
The `schedule` field controls how frequently the [Run Scheduled Audits](/reference/cron-run-scheduled-audits) cron job triggers a new audit for the website.
...
<Info>
  Schedule checks happen inside the `/api/cron/run-scheduled-audits` job. If the job is skipped or fails on a scheduled day, that audit run is missed — it does not catch up the next day.
</Info>
