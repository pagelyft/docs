...
The `tag` parameter is a JSON string encoded when the audit task was submitted by the [Run Scheduled Audits](/reference/cron-run-scheduled-audits) cron job:
...
- [Cron: Run Scheduled Audits](/reference/cron-run-scheduled-audits) — submits OnPage audit tasks that trigger this webhook
